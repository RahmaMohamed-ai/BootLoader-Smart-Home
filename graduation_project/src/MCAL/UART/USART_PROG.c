...
void MUSART2_VidInit ( void ){
...
#if USART2_STATUS == USART1_ENABLE
	...
	CLR_BIT( MUSART2 -> CR2 , 12 ); CLR_BIT( MUSART2 -> CR2 , 13 );

#elif USART2_STOP_BITS == HALF_STOP_BIT

	SET_BIT( MUSART2 -> CR2 , 12 ); CLR_BIT( MUSART2 -> CR2 , 13 );

#elif USART2_STOP_BITS == TWO_STOP_BIT

	CLR_BIT( MUSART2 -> CR2 , 12 ); SET_BIT( MUSART2 -> CR2 , 13 );

#elif USART2_STOP_BITS == ONE_AND_HALF_STOP_BIT

	SET_BIT( MUSART2 -> CR2 , 12 ); SET_BIT( MUSART2 -> CR2 , 13 );
...
#elif USART2_STATUS == USART1_DISABLE
	CLR_BIT( MUSART2 -> CR1 , 0 );
...
void MUSART2_VidSendStringSynch ( u8 * Copy_ptrString ){
	...
	while ( Copy_ptrString[ LOC_u8Iterator ] != '\0' ){
		...
		MUSART2_VidSendCharSync( Copy_ptrString[ LOC_u8Iterator ] );
		LOC_u8Iterator++ ;
...
u8 MUSART2_u8RecCharSynch ( void ){
	...
	while( ( GET_BIT ( MUSART2 -> SR, 5) == 0 ) && ( LOC_TimeOut < THRESHOLD_VALUE ) )
	{
		LOC_TimeOut++;
	}
...
void MUSART2_PtrReadStringSynch ( char *String ){

	u8 LOC_Iterator = 0 , LOC_u8DataCome ;

	while( ( LOC_u8DataCome = MUSART2_u8RecCharSynch() ) != 13 ){
